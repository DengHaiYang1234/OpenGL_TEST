...
int main()
{
    ...
    // --------------------
    sceneShader.use();
    sceneShader.set_uniform("texture1", 0);
    
    fboShader.use();
    fboShader.set_uniform("screenTexture", 0);
...
//        glBindFramebuffer(GL_FRAMEBUFFER, 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        sceneShader.set_uniform("projection", glm::value_ptr(projection));
        sceneShader.set_uniform("view", glm::value_ptr(view));
        // cubes
        glBindVertexArray(cubeVAO);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, cubeTexture);
        model = glm::translate(model, glm::vec3(-1.0f, 0.0f, -1.0f));
        sceneShader.set_uniform("model", glm::value_ptr(model));
