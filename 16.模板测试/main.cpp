...
//    glStencilMask(0x00); // 每一位在写入模板缓冲时都会变成0（禁用写入）
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderSingleColor.set_uniform("view", glm::value_ptr(view));
        shaderSingleColor.set_uniform("projection", glm::value_ptr(projection));
        ...
        shader.set_uniform("model", glm::value_ptr(glm::mat4(1.0f)));
        ...
        shaderSingleColor.set_uniform("model", glm::value_ptr(model));
        glDrawArrays(GL_TRIANGLES, 0, 36);
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
        model = glm::scale(model, glm::vec3(scale, scale, scale));
