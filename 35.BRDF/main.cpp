...
//    ShaderProgram quadShader(ApplicationPath + "35.BRDF/previewVertex.glsl",ApplicationPath + "35.BRDF/previewFragment.glsl");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        pbrShader.set_uniform("projection", glm::value_ptr(projection));
        pbrShader.set_uniform("view", glm::value_ptr(view));
        pbrShader.set_uniform("camPos", camera.Position.x,camera.Position.y,camera.Position.z);
        pbrShader.set_uniform("model", glm::value_ptr(model));
        pbrShader.set_uniform("albedo", 0.5f, 0.0f, 0.0f);
        pbrShader.set_uniform("ao", 1.0f);
        pbrShader.set_uniform("albedoMap", 0);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, albedoTex);
        pbrShader.set_uniform("normalMap", 1);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, normalTex);
        pbrShader.set_uniform("metallicMap", 2);
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, metallicTex);
        pbrShader.set_uniform("roughnessMap", 3);
        ...
        for (unsigned int i = 0; i < sizeof(lightPositions) / sizeof(lightPositions[0]); ++i)
        {
            ...
            pbrShader.set_uniform("lights[" + std::to_string(i) + "].Position", newPos.x,newPos.y,newPos.z);
            pbrShader.set_uniform("lights[" + std::to_string(i) + "].Color", lightColors[i].x,lightColors[i].y,lightColors[i].z);
