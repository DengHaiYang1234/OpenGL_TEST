...
/*
 优点：
    1.存在大量光照时，计算快速；
 缺点：
    1.造成显存压力，通常位置和法线信息至少需要float16存储，并且gBuffer根据需求可能会增加更多；
    2.无法进行MASS，因为光照计算已经在光栅化之后进行的；
    3.无法进行混合，因为G缓冲中所有的数据都是从一个单独的片段中来的，而混合需要对多个片段的组合进行操作；
 */
...
int main(int argc, const char * argv[]) {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (GLuint i = 0; i < objectPositions.size(); i++)
        {
            ...
            gBUfferShader.set_uniform("model", glm::value_ptr(model));
        ...
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        lightingShader.use();
        lightingShader.set_uniform("gPositionTex", 0);
        lightingShader.set_uniform("gNormalTex", 1);
        lightingShader.set_uniform("gColorSpecTex", 2);
        ...
        for (GLuint i = 0; i < lightPositions.size(); i++)
        {
            ...
            lightingShader.set_uniform("lights[" + std::to_string(i) + "].Linear",linear);
            lightingShader.set_uniform("lights[" + std::to_string(i) + "].Quadratic",quadratic);
        }
